...
float Box2DSprite::getCorrectedHeight(){
	return height*std::abs(parents.at(0)->transform->getScaleVector().y)*scale;
}
float Box2DSprite::getCorrectedWidth(){
	return width*std::abs(parents.at(0)->transform->getScaleVector().x)*scale;
}

b2Fixture * Box2DSprite::createFixture(b2Filter _filter, b2Vec2 _offset, void * _userData, bool _isSensor){
	b2PolygonShape tShape;
	glm::vec3 scaleVec = parents.at(0)->transform->getScaleVector();
