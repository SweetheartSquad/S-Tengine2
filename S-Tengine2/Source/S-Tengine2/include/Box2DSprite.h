...
public:
	...
	explicit Box2DSprite(Box2DWorld * _world, TextureSampler * _textureSampler, b2BodyType _bodyType = b2_dynamicBody, bool _defaultFixture = true, Shader* _shader = nullptr, float _componentScale = 0.005f);
	explicit Box2DSprite(Box2DWorld * _world, b2BodyType _bodyType = b2_dynamicBody, bool _defaultFixture = true, Shader * _shader = nullptr, Texture * _texture = nullptr, float _width = 1.f, float _height = 1.f, float _u = 0.f, float _v = 0.f, float _componentScale = 0.005f);
	~Box2DSprite();	
